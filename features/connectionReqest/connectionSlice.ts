...
const connectionSlice = createSlice({
    ...
    reducers: {
        ...
        sendConnection: (state, action: PayloadAction<{ sendUserUid: string; receiverUid: string }>) => {
            state.isLoading = "pending";
        },
        sendConnectionSuccess: (state) => {
            state.isLoading = "succeeded";
        },
        sendConnectionFailure: (state) => {
            state.isLoading = "failed";
        },

        // ─── Get Sent Connection Requests ─────────────────────
        getSentRequests: (state, action: PayloadAction<string>) => {
            state.isLoading = "pending";
        },
        getSentRequestsSuccess: (state, action: PayloadAction<ConnectionRequest[]>) => {
            state.isLoading = "succeeded";
            state.sendConnectionRequest = action.payload;
        },
        getSentRequestsFailure: (state) => {
            state.isLoading = "failed";
        },

        // ─── Get Received Connection Requests ─────────────────
        getReceivedRequests: (state, action: PayloadAction<string>) => {
            state.isLoading = "pending";
        },
        getReceivedRequestsSuccess: (state, action: PayloadAction<ConnectionRequest[]>) => {
            state.isLoading = "succeeded";
            state.receivedConnectionRequest = action.payload;
        },
        getReceivedRequestsFailure: (state) => {
            state.isLoading = "failed";
        },

        // ─── Reject and Remove Connection Request ─────────────
        rejectAndRemoveRequest: (state, action: PayloadAction<{ requestId: string; rejectedUserUid: string }>) => {
            state.isLoading = "pending";
        },
        rejectAndRemoveRequestSuccess: (state, action: PayloadAction<string>) => {
            state.isLoading = "succeeded";
            const sentIndex = state.sendConnectionRequest?.findIndex(
                (request) => request.requestId === action.payload
            );
            if (sentIndex !== undefined && sentIndex !== -1) {
                state.sendConnectionRequest?.splice(sentIndex, 1);
            } else {
                const receivedIndex = state.receivedConnectionRequest?.findIndex(
                    (request) => request.requestId === action.payload
                );
                if (receivedIndex !== undefined && receivedIndex !== -1) {
                    state.receivedConnectionRequest?.splice(receivedIndex, 1);
                }
            }
        },
        rejectAndRemoveRequestFailure: (state) => {
            state.isLoading = "failed";
        },

        // ─── Accept Connection Request ─────────────────────────
        acceptRequest: (state, action: PayloadAction<{ senderUid: string; receiverUid: string }>) => {
            state.isLoading = "pending";
        },
        acceptRequestSuccess: (state, action: PayloadAction<{ senderUid: string }>) => {
            state.isLoading = "succeeded";
            if (state.receivedConnectionRequest) {
                state.receivedConnectionRequest = state.receivedConnectionRequest.filter(
                    (req) => req.uid !== action.payload.senderUid
                );
            }
            if (state.sendConnectionRequest) {
                state.sendConnectionRequest = state.sendConnectionRequest.filter(
        ...
        acceptRequestFailure: (state) => {
            state.isLoading = "failed";
        },

        // ─── Fetch Connections ─────────────────────────────────
        fetchConnectionsList: (state, action: PayloadAction<string>) => {
            state.isLoading = "pending";
        },
        fetchConnectionsListSuccess: (state, action: PayloadAction<ConnectionRequest[]>) => {
            state.isLoading = "succeeded";
            state.connections = action.payload;
        },
        fetchConnectionsListFailure: (state) => {
            state.isLoading = "failed";
        },
    },
});
...
export const {
    sendConnection, sendConnectionSuccess, sendConnectionFailure,
    getSentRequests, getSentRequestsSuccess, getSentRequestsFailure,
    getReceivedRequests, getReceivedRequestsSuccess, getReceivedRequestsFailure,
    rejectAndRemoveRequest, rejectAndRemoveRequestSuccess, rejectAndRemoveRequestFailure,
    acceptRequest, acceptRequestSuccess, acceptRequestFailure,
    fetchConnectionsList, fetchConnectionsListSuccess, fetchConnectionsListFailure,
